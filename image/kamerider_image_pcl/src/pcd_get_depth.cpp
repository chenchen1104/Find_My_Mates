...
private:
    ...
    void yoloCallback (const darknet_ros_msgs::BoundingBoxes& msg){
        ...
        if (msg.bounding_boxes.size() > 1)
        {
            ...
            {
                if (msg.bounding_boxes[i].Class == "driver")
                {
                    posinview.x_offset = msg.bounding_boxes[i].xmin;
                    posinview.y_offset = msg.bounding_boxes[i].ymin;
                    posinview.width    = msg.bounding_boxes[i].xmax - msg.bounding_boxes[i].xmin;
                    posinview.height   = msg.bounding_boxes[i].ymax - msg.bounding_boxes[i].ymin;
                    work = true;
                }
            }
        }
...
public:
    Depth(){
        ...
        posview_sub = nh.subscribe("/darknet_ros/bounding_boxes", 1, &Depth::yoloCallback, this);
